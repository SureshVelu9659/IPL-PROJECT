$(document).ready(function (){
    ...
    $("#searchIco").click(function (){
        ...
        for (var j=0; j< teamsData.length; j++){
            if (teamsData[j].key == searchKey.toUpperCase()){
                ...
                localStorage.setItem("teamData", JSON.stringyfy(teamsData[j]));
                break;
    ...
    $("#playerPlayingStatus").html(playerData.isPlaying ? "Playing" : "On-Beach");
    $("#playerRole").html(playerData.description);
    $("#playerPic img").attr(playerData.imgUrl);
});
